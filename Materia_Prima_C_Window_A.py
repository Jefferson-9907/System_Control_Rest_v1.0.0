...
class Materia_P_C:
    ...
    def __init__(self, root):
        ...
        self.txt = "SYSTEM CONTROL REST® (COMPRA DE MATERIA PRIMA)"
        self.count = 0
        self.text = ''
        ...
        self.Column2.add_command(label='Producto', command=self.producto_btn)
        ...
        self.Column3.add_command(label='Materia Prima')
        ...
        self.e_id_compra_1 = StringVar()
        self.e_id_materia_p_1 = StringVar()
        self.e_id_proveedor_1 = StringVar()
        self.e_cantidad_1 = StringVar()
        self.e_precio_1 = StringVar()
        self.e_observacion_1 = StringVar()
        self.search_entry = StringVar()

        m_title = Label(Manage_Frame, text="-ADM. COMPRA MATERIA PRIMA-", font=("Copperplate Gothic Bold", 16, "bold"),
                        bg='#a27114', fg="White")
        m_title.grid(row=0, columnspan=2, padx=10, pady=50)

        self.l_id_c_materia_p = Label(Manage_Frame, text='CÓD.', width='15', font=('Copperplate Gothic Bold', 10),
                                      bg='#808080')
        self.l_id_c_materia_p.grid(column=0, row=1, padx=0, pady=5)
        self.e_id_c_materia_p = Entry(Manage_Frame, textvariable=self.e_id_compra_1, width=33, state="disabled")
        self.e_id_c_materia_p.grid(column=1, row=1, padx=0, pady=5, sticky="W")

        with sqlite3.connect('ddbb_sys_rest.db') as conexion:
            try:  # Captura la excepcion en caso de que algo falle
                cursor = conexion.cursor()
                cursor.execute('SELECT * FROM INV_MATERIA_P')
        ...
        self.l_id_materia_p = Label(Manage_Frame, text='MATERIA PRIMA', width='15',
                                    font=('Copperplate Gothic Bold', 10), bg='#808080')
        self.l_id_materia_p.grid(column=0, row=2, padx=1, pady=5)
        self.e_id_materia_p = Combobox(Manage_Frame, textvariable=self.e_id_materia_p_1, width=20)
        self.e_id_materia_p.bind('<Return>', self.sig_entry)
        self.e_id_materia_p['value'] = result
        self.e_id_materia_p.grid(column=1, row=2, padx=1, pady=5, sticky="W")

        self.l_cantidad = Label(Manage_Frame, text='CANTIDAD', width='15', font=('Copperplate Gothic Bold', 10),
                                bg='#808080')
        self.l_cantidad.grid(column=0, row=3, padx=0, pady=5)
        self.e_cantidad = Entry(Manage_Frame, textvariable=self.e_cantidad_1, width=33)
        self.e_cantidad.grid(column=1, row=3, padx=0, pady=5, sticky="W")

        self.l_precio = Label(Manage_Frame, text='CANTIDAD', width='15', font=('Copperplate Gothic Bold', 10),
                              bg='#808080')
        self.l_precio.grid(column=0, row=3, padx=0, pady=5)
        self.e_precio = Entry(Manage_Frame, textvariable=self.e_precio_1, width=33)
        self.e_precio.grid(column=1, row=3, padx=0, pady=5, sticky="W")

        self.l_observacion = Label(Manage_Frame, text='OBSERV.', width='15', font=('Copperplate Gothic Bold', 10),
                                   bg='#808080')
        self.l_observacion.grid(column=0, row=4, padx=1, pady=5)
        self.e_observacion = Entry(Manage_Frame, textvariable=self.e_observacion_1, width=33)
        self.e_observacion_1.set("S/O")
        self.e_observacion.grid(column=1, row=4, padx=1, pady=5, sticky="W")
    ...
    def update(self):
        materia_prima = self.m_prima
        materia_prima.descripcion = self.e_descripcion_1.get()
        materia_prima.cantidad = self.e_cantidad_1.set('0')
        materia_prima.observacion = self.e_observacion_1.get()
