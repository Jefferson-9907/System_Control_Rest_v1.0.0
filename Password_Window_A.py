...
class Password:
    def __init__(self, root):
        self.root = root
        self.root.title("SYST_CONTROL(REST®)--›(CONTRASEÑA)")
        ...
        self.txt = "SYSTEM CONTROL REST® (CAMBIAR CONTRASEÑA)"
        self.count = 0
        self.text = ''
        ...
        self.Column2.add_command(label='Proveedor_W', command=self.proveedor_btn)
    ...
    def facturation_btn(self):
        root = Toplevel()
        Facturation_Window_A.Facturation(root)
        self.root.withdraw()
        root.deiconify()
