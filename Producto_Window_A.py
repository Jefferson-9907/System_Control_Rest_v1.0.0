...
class Producto_w:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("SYST_CONTROL(REST®)--›(PRODUCTO)")
        ...
        self.txt = "SYSTEM CONTROL REST® (PRODUCTOS)"
        self.count = 0
        self.text = ''
        ...
        self.Column1.add_command(label='Producto')
        ...
        self.Column6.add_command(label='Generar Reportes')
        ...
        self.Column7.add_command(label='Sobre SIST_CONTROL (REST®)', command=self.caja_info_sist)
        self.Column7.add_separator()
        self.root.config(menu=self.menus)
        ...
        self.e_id_producto_1 = StringVar()
        self.e_descripcion_1 = StringVar()
        self.e_valor_c_1 = DoubleVar()
        self.e_valor_v_1 = DoubleVar()
        self.e_stock_1 = IntVar()
        self.e_estado_1 = StringVar()

        self.search_entry = StringVar()
        self.e_valor_c_1.set(0.00)
        self.e_valor_v_1.set(0.00)

        m_title = Label(Manage_Frame, text="-ADMINISTAR PRODUCTOS-", font=("Copperplate Gothic Bold", 16, "bold"),
                        bg='#a27114', fg="White")
        m_title.grid(row=0, columnspan=2, padx=10, pady=50)

        self.l_id_producto = Label(Manage_Frame, text='CÓD.', width='15', font=('Copperplate Gothic Bold', 10),
                                   bg='#808080')
        self.l_id_producto.grid(column=0, row=1, padx=0, pady=5)
        self.e_id_producto = Entry(Manage_Frame, textvariable=self.e_id_producto_1, width=33)
        self.e_id_producto.focus()
        self.e_id_producto.bind('<Return>', self.sig_entry)
        self.e_id_producto.grid(column=1, row=1, padx=0, pady=5, sticky="W")

        self.l_descripcion = Label(Manage_Frame, text='DESCRIPCIÓN', width='15', font=('Copperplate Gothic Bold', 10),
                                   bg='#808080')
        ...
        self.e_descripcion.bind('<Return>', self.sig_entry1)
        self.e_descripcion.grid(column=1, row=2, padx=1, pady=5, sticky="W")

        self.l_valor_c = Label(Manage_Frame, text='VALOR COMPRA: $', width='15', font=('Copperplate Gothic Bold', 10),
                               bg='#808080')
        self.l_valor_c.grid(column=0, row=3, padx=0, pady=5)
        self.e_valor_c = Entry(Manage_Frame, textvariable=self.e_valor_c_1, width=10)
        self.e_valor_c.bind('<Return>', self.sig_entry2)
        self.e_valor_c.grid(column=1, row=3, padx=0, pady=5, sticky="W")

        self.l_valor_v = Label(Manage_Frame, text='VALOR VENTA: $', width='15', font=('Copperplate Gothic Bold', 10),
                               bg='#808080')
        self.l_valor_v.grid(column=0, row=4, padx=0, pady=5)
        self.e_valor_v = Entry(Manage_Frame, textvariable=self.e_valor_v_1, width=10)
        self.e_valor_v.bind('<Return>', self.sig_entry3)
        self.e_valor_v.grid(column=1, row=4, padx=0, pady=5, sticky="W")

        self.l_stock = Label(Manage_Frame, text='STOCK', width='15', font=('Copperplate Gothic Bold', 10),
                             bg='#808080')
        self.l_stock.grid(column=0, row=5, padx=0, pady=5)
        self.e_stock = Entry(Manage_Frame, textvariable=self.e_stock_1, width=33, state="disabled")
        self.e_stock.grid(column=1, row=5, padx=0, pady=5, sticky="W")

        self.estado = Label(Manage_Frame, text='ESTADO :', width='15', font=("Copperplate Gothic Bold", 10),
                            bg='#808080')
        self.estado.grid(column=0, row=6, padx=1, pady=5)
        self.valor = BooleanVar()
        self.txtEstado = Checkbutton(Manage_Frame, variable=self.valor, onvalue=True, offvalue=False,
                                     bg='#a27114')
        self.txtEstado.grid(column=1, row=6, padx=1, pady=5, sticky="W")

        # Button Frame
        self.btn_frame = Frame(Manage_Frame, bg='#a27114')
        self.btn_frame.place(x=10, y=500, width=360)

        self.add_btn = Button(self.btn_frame, image=self.imagenes['nuevo'], text='REGISTAR', command=self.add_producto,
                              compound=TOP)
        ...
        self.Table = ttk.Treeview(Table_Frame, columns=("cod", "des", "val", "est"),
        ...
        self.Table.heading("cod", text="COD.")
        self.Table.heading("des", text="DESCRIPCIÓN")
        self.Table.heading("val", text="VALOR")
        self.Table.heading("est", text="ESTADO")

        self.Table['show'] = "headings"
        self.Table.column("cod", width=100)
        self.Table.column("des", width=150)
        self.Table.column("val", width=150)
        self.Table.column("est", width=50)
    ...
    def sig_entry(self, event):
        if self.e_id_producto_1.get() == '':
            messagebox.showwarning("SYST_CONTROL(IFAP®)-->(ADVERTENCIA)", "INGRESE EL CAMPO: COD.")
            self.e_id_producto.focus()
    ...
    def sig_entry1(self, event):
        ...
        if self.e_descripcion.get() == '':
            messagebox.showwarning("SYST_CONTROL(IFAP®)-->(ADVERTENCIA)", "INGRESE EL CAMPO: No. DESCRIPCIÓN!!!")
            self.e_descripcion.focus()

        elif a.isnumeric():
            messagebox.showwarning("SYST_CONTROL(IFAP®)-->(ADVERTENCIA)", "NO SE ADMITEN NÚMEROS EN EL CAMPO: "
                                                                          "DESCRIPCIÓN")
            self.e_descripcion_1.set("")
            self.e_descripcion.focus()

        elif a.isspace():
            messagebox.showwarning("SYST_CONTROL(IFAP®)-->(ADVERTENCIA)", "NO SE PERMITEN ESPACIOS EN EL CAMPO: "
                                                                          "DESCRIPCIÓN")
            self.e_descripcion_1.set("")
            self.e_descripcion.focus()

        else:
            self.e_valor_c.focus()

    def sig_entry2(self, event):
        if self.e_valor_c_1.get() == '':
            messagebox.showwarning("SYST_CONTROL(IFAP®)-->(ADVERTENCIA)", "INGRESE EL CAMPO: VALOR!!!")
            self.e_valor_c.focus()

        else:
            self.e_valor_v.focus()

    def sig_entry3(self, event):
        if self.e_valor_v_1.get() == '':
            messagebox.showwarning("SYST_CONTROL(IFAP®)-->(ADVERTENCIA)", "INGRESE EL CAMPO: VALOR!!!")
            self.e_valor_v.focus()
    ...
    def add_producto(self):
        if self.e_id_producto_1.get() == '' or self.e_descripcion_1.get() == '' or self.e_valor_c_1.get() == '' or \
                self.e_valor_v_1.get() == '' or self.e_estado_1.get() == '':
    ...
    def click_submit(self):
        ...
        producto.id = self.e_id_producto_1.get()
        producto.nombre = self.e_descripcion_1.get()
        producto.precio_compra = self.e_valor_c_1.get()
        producto.precio_venta = self.e_valor_v_1.get()
        producto.stock = 0
        producto.estado = self.valor.get()
        producto.guardar()

    def clear_field(self):
        self.e_id_producto_1.set('')
        self.e_descripcion_1.set('')
        self.e_valor_c_1.set(0.00)
        self.e_valor_v_1.set(0.00)
        self.e_estado_1.set('')
        self.e_stock_1.set(0)
        self.e_descripcion.focus()
    ...
    def get_fields(self, row):
        ...
        self.e_id_producto_1.set(str(row[0]))
        self.e_descripcion_1.set(row[1])
        self.e_valor_c_1.set(float(row[2]))
        self.e_valor_v_1.set(float(row[3]))
        self.e_estado_1.set(row[4])
    ...
    def search_data(self):
        ...
        if self.search_entry.get() != '':
                        ...
                        if self.output:
                            ...
                            with sqlite3.connect('ddbb_sys_rest.db') as conexion:
                                try:  # Captura la excepcion en caso de que algo falle
                                    cursor = conexion.cursor()
                                    data = cursor.execute(
                                        "SELECT * FROM INV_PRODUCTO WHERE descripcion LIKE '%" + \
            ...
            messagebox.showerror("SYST_CONTROL(REST®)-->(ADVERTENCIA)", "EL CAMPO DE BÚSQUEDA SE ENCUENTRA VACÍO\n"
    ...
    def show_data(self):
        with sqlite3.connect('ddbb_sys_rest.db') as conexion:
            try:  # Captura la excepcion en caso de que algo falle
                cursor = conexion.cursor()
                data = cursor.execute("SELECT * FROM INV_PRODUCTO")
