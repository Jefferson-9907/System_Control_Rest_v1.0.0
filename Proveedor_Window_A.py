...
class Proveedor_W:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("SYST_CONTROL(REST®)--›(PROVEEDOR)")
        ...
        self.txt = "SYSTEM CONTROL REST® (PROVEEDOR)"
        self.count = 0
        self.text = ''
        ...
        self.Column3.add_command(label='Proveedor')
        ...
        self.provd = Proveedor()

        # Manage Frame
        Manage_Frame = Frame(self.root, relief=RIDGE, bd=4, bg='#a27114')
        Manage_Frame.place(x=15, y=85, width=385, height=605)

        self.e_id_pr_1 = StringVar()
        self.e_nombres_1 = StringVar()
        self.e_apellidos_1 = StringVar()
        self.e_direccion_1 = StringVar()
        self.e_celular_1 = StringVar()
        self.search_entry = StringVar()

        m_title = Label(Manage_Frame, text="-ADMINISTRAR PROVEEDOR-", font=("Copperplate Gothic Bold", 16, "bold"),
                        bg='#a27114', fg="White")
        m_title.grid(row=0, columnspan=2, padx=10, pady=50)

        self.l_id_pr = Label(Manage_Frame, text='No. CÉDULA:', width='15', font=('Copperplate Gothic Bold', 10),
                             bg='#808080')
        self.l_id_pr.grid(column=0, row=1, padx=0, pady=5)
        self.e_id_pr = Entry(Manage_Frame, textvariable=self.e_id_pr_1, width=33)
        self.e_id_pr.bind('<Return>', self.sig_entry)
        self.e_id_pr.grid(column=1, row=1, padx=0, pady=5, sticky="W")
        self.e_id_pr.focus()

        self.l_nombres = Label(Manage_Frame, text='PROVEEDOR:', width='15', font=('Copperplate Gothic Bold', 10),
                               bg='#808080')
        self.l_nombres.grid(column=0, row=2, padx=1, pady=5)
        self.e_nombres = Entry(Manage_Frame, textvariable=self.e_nombres_1, width=33)
        self.e_nombres.bind('<Return>', self.sig_entry1)
        self.e_nombres.grid(column=1, row=2, padx=1, pady=5, sticky="W")

        self.l_direccion = Label(Manage_Frame, text='DIRECCIÓN:', width='15', font=('Copperplate Gothic Bold', 10),
                                 bg='#808080')
        self.l_direccion.grid(column=0, row=3, padx=1, pady=5)
        self.e_direccion = Entry(Manage_Frame, textvariable=self.e_direccion_1, width=15)
        self.e_direccion.bind('<Return>', self.sig_entry2)
        self.e_direccion.grid(column=1, row=3, padx=1, pady=5, sticky="W")

        self.l_celular = Label(Manage_Frame, text='No. CELULAR:', width='15', font=('Copperplate Gothic Bold', 10),
                               bg='#808080')
        self.l_celular.grid(column=0, row=4, padx=1, pady=5)
        self.e_celular = Entry(Manage_Frame, textvariable=self.e_celular_1, width=15)
        self.e_celular.grid(column=1, row=4, padx=1, pady=5, sticky="W")

        # Button Frame
        self.btn_frame = Frame(Manage_Frame, bg='#a27114')
        self.btn_frame.place(x=10, y=500, width=360)

        self.add_btn = Button(self.btn_frame, image=self.imagenes['nuevo'], text='REGISTAR', command=self.add_proveedor,
                              compound=TOP)
        ...
        self.Table = ttk.Treeview(Table_Frame, columns=("cod", "nom", "cel", "dir"),
        ...
        self.Table.heading("cod", text="No. CÉDULA")
        self.Table.heading("nom", text="PROVEEDOR")
        self.Table.heading("cel", text="No. CELULAR")
        self.Table.heading("dir", text="DIRECCIÓN")

        self.Table['show'] = "headings"
        self.Table.column("cod", width=10)
        self.Table.column("nom", width=150)
        self.Table.column("cel", width=10)
    ...
    def sig_entry(self, event):
        a = str(self.e_id_pr_1.get())
        if self.e_id_pr_1.get() == '':
            messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)", "INGRESE EL CAMPO: No. DE CÉDULA!!!")
            self.e_id_pr.focus()
            ...
            messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)", "No. DE CÉDULA NO VÁLIDO\n"
                                                                          "CONTIENE MÁS DE 1O DÍGITOS!!!")
            self.e_id_pr_1.set('')
            self.e_id_pr.focus()

        elif len(a) < 10:
            messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)", "No. DE CÉDULA NO VÁLIDO\n"
                                                                          "CONTIENE MENOS DE 1O DÍGITOS!!!")
            self.e_id_pr_1.set('')
            self.e_id_pr.focus()
    ...
    def sig_entry1(self, event):
        ...
        if self.e_nombres.get() == '':
            ...
            messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)", "NO SE ADMITEN NÚMEROS EN EL CAMPO: "
                                                                          "NOMBRES")
            self.e_nombres_1.set("")
            self.e_nombres.focus()

        elif a.isspace():
            messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)",
                                   "NO SE PERMITEN ESPACIOS EN EL CAMPO: NOMBRES")
            self.e_nombres_1.set("")
            self.e_nombres.focus()
    ...
    def sig_entry2(self, event):
        a = self.e_direccion_1.get()
        if self.e_direccion_1.get() == '':
    ...
    def sig_entry3(self, event):
        a = self.e_celular_1.get()
        if self.e_celular.get() == '':
            ...
            messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)", "NO SE ADMITEN NÚMEROS EN EL CAMPO: "
                                                                          "No. CELULAR")
            self.e_celular_1.set("")
            self.e_celular.focus()

        elif len(a) > 10:
            messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)", "No. CELULAR NO VÁLIDO\n"
                                                                          "CONTIENE MÁS DE 1O DÍGITOS!!!")
            self.e_celular_1.set("")
            self.e_celular.focus()
    ...
    def add_proveedor(self):
        if self.e_id_pr_1.get() == "" or self.e_nombres_1.get() == "" or self.e_direccion_1.get() == "" or \
                self.e_celular_1.get() == "":
    ...
    def click_submit(self):
        self.proveedor = self.e_nombres_1.get() + " " + self.e_apellidos_1.get()
        """
        ...
        """
        proveedor = self.provd
        proveedor.id = self.e_id_pr_1.get()
        proveedor.nombres = self.proveedor
        proveedor.n_celular = self.e_celular_1.get()
        proveedor.direccion = self.e_direccion_1.get()
        proveedor.guardar()
        self.show_data()
        self.clear_field()
    ...
    def get_fields(self, row):
        ...
        self.e_id_pr_1.set(str(row[0]))
        self.e_nombres_1.set(row[1])
        self.e_celular_1.set(row[2])
        self.e_direccion_1.set(row[3])
        self.add_btn["state"] = "disabled"
        self.up_btn["state"] = "normal"
        self.del_btn["state"] = "normal"

    def validation(self):
        if self.e_nombres_1.get() == '' or self.e_direccion_1.get() == '' or self.e_celular_1.get() == '':
            messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)", "TODOS LOS CAMPOS SON OBLIGATORIOS!!!")
    ...
    def update(self):
        proveedor = self.provd
        proveedor.nombres = self.e_nombres_1.get() + " " + self.e_apellidos_1.get()
        proveedor.n_celular = self.e_celular_1.get()
        proveedor.direccion = self.e_direccion_1.get()
        proveedor.id = self.e_id_pr_1.get()
    ...
    def search_data(self):
        ...
        if self.search_entry.get() != '':
            if a.isnumeric():
                messagebox.showerror("SYST_CONTROL(REST®)-->(ADVERTENCIA)",
                                     "NO SE ADMITEN NÚMEROS EN EL CAMPO DE BÚSQUEDA DE PROVEEDOR")
                self.search_entry.set("")
            elif a.isspace():
                messagebox.showwarning("SYST_CONTROL(REST®)-->(ADVERTENCIA)",
                                       "NO SE ADMITEN ESPACIOS EN EL CAMPO DE BÚSQUEDA DE PROVEEDOR")
                self.search_entry.set("")
            else:
                if a.isalpha():
                    try:
                        ...
                        if self.output:
                            messagebox.showinfo("SYST_CONTROL(REST®)-->(ENCONTRADO)",
                                                f"EL PROVEEDOR: '{self.output}' HA SIDO ENCONTRADO")

                            with sqlite3.connect('ddbb_sys_rest.db') as conexion:
                                try:  # Captura la excepcion en caso de que algo falle
                                    cursor = conexion.cursor()
                                    data = cursor.execute(
                                        "SELECT * FROM INV_PROVEEDOR WHERE nombres LIKE '" + str(self.output) + "%'")
    ...
    def show_data(self):
        with sqlite3.connect('ddbb_sys_rest.db') as conexion:
            try:  # Captura la excepcion en caso de que algo falle
                cursor = conexion.cursor()
                data = cursor.execute('SELECT id_proveedor, nombres, n_celular, direccion FROM INV_PROVEEDOR')
